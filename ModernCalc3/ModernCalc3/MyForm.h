...
#pragma endregion
	...
	private: bool secondValTime = false;
	...
	private: Void triggerOperator(String^ oper) {
		...
		this->cashSecond = "";
		if (this->cashFirst != "" && this->cashOperator != "" && !this->secondValTime) {
			calcResult();
		}
		else {
	...
	private: Void calcResult() {
		...
		if (this->input->Text != "" && this->cashSecond == "" && !this->afterEquals) {
	...
	private: System::Void sqrt_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		if (this->input->Text->StartsWith("-")) {
			showMessage("Подкорневое выражение не должно быть отрицательным.");
			return;
		}
