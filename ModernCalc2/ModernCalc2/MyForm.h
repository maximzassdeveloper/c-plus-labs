...
namespace ModernCalc {
	...
	private: String^ cashFirst = "";
	private: String^ cashSecond = "";
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			// cashInput
			// 
			this->cashInput->Location = System::Drawing::Point(0, 0);
			this->cashInput->Name = L"cashInput";
			this->cashInput->Size = System::Drawing::Size(100, 23);
...
#pragma endregion
	...
	private: Void triggerOperator(String^ oper) {
		...
		if (this->cashFirst != "" && this->cashOperator != "") {
			String^ res = calcResult();
			if (res == "") {
				return;
			}
			this->cashFirst = res;
	...
	// Проверка, чтобы значение не заканчивалось на запятую или минус
	private: bool validate() {
		String^ text = this->input->Text;
		if (text == "") {
			showMessage("Значение не должно быть пустым.");
			return false;
		}
		if (text->EndsWith(",")) {
	...
	private: String^ calcResult() {
		...
		String^ res = Convert::ToString(operatorSwitch(this->cashOperator, this->cashFirst, this->cashSecond));
		// this->input->Text = res;
		// this->cashFirst = res;
		return res;
	}
